...
router.put("/:id", authenticate, async (req, res, next) => {
    if (!req.isAdmin) return next(new ResponseError("Only admins may update lenders", HTTP_STATUS_CODES.FORBIDDEN));

    const { error } = lenderUpdateValidation(req.body);
    if (error) return next(new ResponseError(error.details[0].message, HTTP_STATUS_CODES.BAD_REQUEST));

    try {
        const lender = await DbService.getById(COLLECTIONS.LENDERS, req.params.id);
        if (!lender) return next(new ResponseError("Lender not found", HTTP_STATUS_CODES.NOT_FOUND));

        await DbService.update(COLLECTIONS.LENDERS, { _id: mongoose.Types.ObjectId(req.params.id) }, req.body);
...
router.post("/:id/stripe/account-link/:accountLinkType", authenticate, async (req, res, next) => {
    if (!req.isAdmin) return next(new ResponseError("Only admins may create Stripe account links for lenders", HTTP_STATUS_CODES.FORBIDDEN));
...
router.get("/:id/stripe/account", authenticate, async (req, res, next) => {
    if (!req.isAdmin) return next(new ResponseError("Only admins may get Stripe accounts of lenders", HTTP_STATUS_CODES.FORBIDDEN));
